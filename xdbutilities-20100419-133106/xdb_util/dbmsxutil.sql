Rem
Rem $Header: tk_objects/tkxm/sql/dbmsxutil_otn.sql /main/1 2010/10/07 15:52:46 bhammers Exp $
Rem
Rem dbmsxutil.sql
Rem
...
select s.xmldata.schema_owner OWNER,
       ...
       s.xmldata.schema_url XML_SCHEMA_URL
...
as
select TARGET_NAMESPACE, XML_SCHEMA_URL
...
(select s.xmldata.schema_owner AS OWNER,
       s.xmldata.schema_url AS XML_SCHEMA_URL,
       s.xmldata.target_namespace AS TARGET_NAMESPACE, 
       e.xmldata.property.name AS ELEMENT_NAME, 
       ...
       hextoraw(ep.xmldata.property.prop_number) AS PARENT_ELEMENT_ID
  from xdb.xdb$schema s, xdb.xdb$element e, xdb.xdb$element ep,
       xdb.xdb$complex_type ct, xdb.xdb$sequence_model cm,
       table(cm.xmldata.elements) 
refToKids
 where sys_op_r2o(e.xmldata.property.parent_schema) = s.sys_nc_oid$ and
       ref(ct) = ep.xmldata.property.type_ref and
       sys_op_r2o(ct.xmldata.sequence_kid) = cm.sys_nc_oid$ and
       refToKids.column_value = ref(e)
UNION ALL
select s.xmldata.schema_owner AS OWNER,
       s.xmldata.schema_url AS XML_SCHEMA_URL,
       s.xmldata.target_namespace AS TARGET_NAMESPACE, 
       ...
       hextoraw(ep.xmldata.property.prop_number) AS PARENT_ELEMENT_ID
  from xdb.xdb$schema s, xdb.xdb$element e, xdb.xdb$element ep,
       xdb.xdb$complex_type ct, xdb.xdb$choice_model cm,
       table(cm.xmldata.elements) 
refToKids
 where sys_op_r2o(e.xmldata.property.parent_schema) = s.sys_nc_oid$ and
       ref(ct) = ep.xmldata.property.type_ref and
       sys_op_r2o(ct.xmldata.choice_kid) = cm.sys_nc_oid$ and
       refToKids.column_value = ref(e)
UNION ALL
select s.xmldata.schema_owner AS OWNER,
       s.xmldata.schema_url AS XML_SCHEMA_URL,
       ...
       hextoraw(ep.xmldata.property.prop_number) AS PARENT_ELEMENT_ID
  from xdb.xdb$schema s, xdb.xdb$element e, xdb.xdb$element ep,
       xdb.xdb$complex_type ct, xdb.xdb$all_model cm,
       table(cm.xmldata.elements) refToKids
 where sys_op_r2o(e.xmldata.property.parent_schema) = s.sys_nc_oid$ and
       ref(ct) = ep.xmldata.property.type_ref and
       sys_op_r2o(ct.xmldata.all_kid) = cm.sys_nc_oid$ and
       refToKids.column_value = ref(e)
UNION ALL
select s.xmldata.schema_owner AS OWNER,
       s.xmldata.schema_url AS XML_SCHEMA_URL,
...
from
xdb.xdb$schema s, xdb.xdb$element e
where 
sys_op_r2o(e.xmldata.property.parent_schema) = s.sys_nc_oid$ and
ref(e) IN
 ( select ref(e)  
        from  xdb.xdb$element e
       MINUS 
       select 
        value(seqKids)  ELEMENT_NAME
       from
       xdb.xdb$sequence_model cms, table(cms.xmldata.elements) seqKids 
       MINUS 
       select 
        value(choiceKids)  ELEMENT_NAME
       from
       xdb.xdb$choice_model cmc, table(cmc.xmldata.elements) choiceKids 
       MINUS 
       select 
        value(allKids)  ELEMENT_NAME
       from
       xdb.xdb$all_model cma, table(cma.xmldata.elements) allKids 
 ) 
) 
/
...
select s.xmldata.schema_owner AS OWNER,
       ...
       NULL AS PARENT_ELEMENT_ID
from
xdb.xdb$schema s, xdb.xdb$element e, all_xml_schemas a
where 
sys_op_r2o(e.xmldata.property.parent_schema) = s.sys_nc_oid$ and
s.xmldata.schema_owner = a.owner and 
s.xmldata.schema_url = a.schema_url and 
...
as
select XML_SCHEMA_URL, TARGET_NAMESPACE, ELEMENT_NAME, TYPE_NAME, GLOBAL, 
...
select s.xmldata.schema_owner OWNER,
 ...
 where sys_op_r2o(e.xmldata.property.parent_schema) = s.sys_nc_oid$
   ...
   and sys_op_r2o(he.xmldata.property.parent_schema) = s.sys_nc_oid$;
/
...
as
select XML_SCHEMA_URL, TARGET_NAMESPACE, ELEMENT_NAME, ELEMENT, HEAD_OWNER, 
...
as
select XML_SCHEMA_URL, TARGET_NAMESPACE, ELEMENT_NAME, ELEMENT
...
select s.xmldata.schema_owner OWNER,
       ...
       extractValue(value(ct),
        '/xs:complexType/@xdb:maintainOrder',
        'xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"') MAINTAIN_ORDER,
       ct.xmldata.sqltype SQL_TYPE,
       ct.xmldata.SQLSCHEMA SQL_SCHEMA,
       extractValue(value(ct),
        '/xs:complexType/@xdb:defaultTable',
        'xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"') DEFAULT_TABLE,
       extractValue(value(ct),
        '/xs:complexType/@xdb:SQLName',
        'xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"') SQL_NAME,
       extractValue(value(ct),
        '/xs:complexType/@xdb:SQLCollType',
        'xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"') SQL_COL_TYPE,
       extractValue(value(ct),
        '/xs:complexType/@xdb:storeVarrayAsTable',
        'xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"') STORE_VARRAY_AS_TABLE
...
as
select XML_SCHEMA_URL, TARGET_NAMESPACE, COMPLEX_TYPE_NAME, COMPLEX_TYPE,
       BASE_NAME, MAINTAIN_DOM,MAINTAIN_ORDER, SQL_TYPE, SQL_SCHEMA, 
       DEFAULT_TABLE, SQL_NAME, SQL_COL_TYPE, STORE_VARRAY_AS_TABLE
  ...
  $IF DBMS_DB_VERSION.VER_LE_10_2 $THEN
    ...
    select s.xmldata.schema_owner OWNER,
       ...
       extractValue(value(st),
        ''/xs:complexType/@xd:maintainDOM'',
        ''xmlns:xs="http://www.w3.org/2001/XMLSchema"
        xmlns:xd="http://xmlns.oracle.com/xdb"'')MAINTAIN_DOM,
        extractValue(value(st),
        ''/xs:complexType/@xdb:SQLSchema'',
        ''xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"'') SQL_SCHEMA,
       extractValue(value(st),
        ''/xs:complexType/@xdb:SQLName'',
        ''xmlns:xs="http://www.w3.org/2001/XMLSchema"
         xmlns:xdb="http://xmlns.oracle.com/xdb"'') SQL_NAME,
...
BEGIN
  $IF DBMS_DB_VERSION.VER_LE_10_2 $THEN
    stmt := '
     ...
     as
     select XML_SCHEMA_URL, TARGET_NAMESPACE, SIMPLE_TYPE_NAME, SIMPLE_TYPE, 
          MAINTAIN_DOM, SQL_SCHEMA, SQL_NAME, SQL_COL_TYPE, 
          STORE_VARRAY_AS_TABLE
  ...
  $ELSE  -- (11.1 or 11.2)
    stmt := '
     ...
     as
     select XML_SCHEMA_URL, TARGET_NAMESPACE, SIMPLE_TYPE_NAME, SIMPLE_TYPE, 
          MAINTAIN_DOM, SQL_TYPE, SQL_SCHEMA, SQL_NAME, SQL_COL_TYPE, 
          STORE_VARRAY_AS_TABLE
     from DBA_XML_SCHEMA_SIMPLE_TYPES
     where OWNER = USER';
...
select s.xmldata.schema_owner OWNER,
       s.xmldata.schema_url XML_SCHEMA_URL,
       s.xmldata.target_namespace TARGET_NAMESPACE, 
       a.xmldata.name ATTRIBUTE_NAME, 
       a.xmldata.typename.name TYPE_NAME,
...
as
select XML_SCHEMA_URL, TARGET_NAMESPACE, ATTRIBUTE_NAME, TYPE_NAME, 
       GLOBAL,ATTRIBUTE, ELEMENT_ID
...
$ELSE --11.2
  function xpath2TabColMapping(owner_name VARCHAR2 default user,
                               ...
                               xpath IN VARCHAR2,
                               namespaces IN VARCHAR2) RETURN XMLTYPE;  
$END
